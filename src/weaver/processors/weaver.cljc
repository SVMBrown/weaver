...
(defmethod pre-process-node [:vector "weaver"] [_ node]
  (pre-process-weaver-vec node))

(defmethod pre-process-node [:keyword "weaver"] [_ node]
...
(defmethod process-node :weaver/drop-nils [_ {:keys [coll]}]
  (let [result (remove nil? coll)]
    (cond
      ...
      (seq? coll) result
      :else (do (x/warn "Unrecognized collection type, returning vec")
                (vec result)))))
